...
rev run <loop>          drive one loop: idle on Helm's event cursor (zero tokens),
                            wake on ready work, spawn one fresh agent session per iteration
rev status              every loop at a glance
rev stop|resume|pace    control verbs (sentinel writes)
