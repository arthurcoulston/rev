#!/bin/bash
# auth-bisect.sh — pin down why a Rev-spawned claude session reads as
...
# re-run this script and scripts/real-smoke.sh. Cases 2-4 isolate Rev's
...
if ! echo "$OUT" | grep -q '"is_error":false'; then
  echo
  echo "VERDICT: the claude CLI itself is not logged in — Rev is not the problem."
  echo "Fix: run \`claude\` interactively, /login, then re-run this script."
  exit 1
fi

echo "2) spawned from node (inherited env, piped stdio) — Rev-style minus scrub:"
...
echo "3) spawned from node with Rev cleanEnv:"
...
echo "4) cleanEnv + stdin ignored (the shipped Rev posture):"
