...
function setup(loopToml: string): Env {
  const home = mkdtempSync(join(tmpdir(), 'capstan-e2e-'));
  const db = join(home, 'helm.db');
  writeFileSync(
    join(home, 'roster.toml'),
    `[global]
helm_cli = "${HELM_CLI}"
helm_mcp_server = "${join(HELM, 'dist', 'server.js')}"
helm_db = "${db}"
poll_seconds = 1
fail_cap = 1
${loopToml}`,
...
function capstan(e: Env, args: string[]): string {
  return execFileSync('npx', ['tsx', CAPSTAN_CLI, ...args], { env: e.env, encoding: 'utf8', cwd: join(import.meta.dirname, '..') });
}

function seedTicket(e: Env, title: string): string {
  return (helm(e, ['create', '--title', title, '--body', 'test work: claim me, complete me', '--workstream', 'capstan-test', '--type', 'ops']) as { id: string }).id;
}

describe('capstan e2e (mock runtime, real helm store)', () => {
...
mock_cmd = '''
set -e
ID=$(node ${HELM_CLI} list --ready --workstream capstan-test --limit 1 | node -e "process.stdin.on('data',d=>{const j=JSON.parse(d);console.log(j.tickets[0]?.id??'')})")
if [ -n "$ID" ]; then
...
`);
    ...
    const out = capstan(e, ['run', 'bad-loop', '--count', '5']);
    expect(out).toContain('BLOCKED');

    expect(existsSync(join(e.home, 'state', 'bad-loop', 'BLOCKED'))).toBe(true);
    // The escalation is a real awaiting_human ticket, filed by the capstan actor.
...
`);
    ...
    const stopped = capstan(e, ['run', 'app-loop']);
    ...
    capstan(e, ['resume', 'app-loop']);
    const out = capstan(e, ['run', 'app-loop', '--count', '1']);
    expect(out).toContain('BLOCKED');
